...
- 将个人磁盘扫描脚本重构为模块化桌面与 CLI 工具，完成全量遍历、候选评分、Top-N 筛选和可视化报告。
- 设计“本地保护规则 → 隐私脱敏 → AI 批量建议 → 严格校验 → 用户确认”的安全混合流程，限制模型直接操作高风险文件。
- 同时适配 Responses 与 Chat Completions 协议，使用 SQLite 实现建议缓存，并加入批量请求、429/5xx 指数退避重试、失败批次拆分和 token 统计。
- 在清理前基于大小、修改时间和文件标识进行状态复核，避免扫描与执行之间文件被修改或替换。
